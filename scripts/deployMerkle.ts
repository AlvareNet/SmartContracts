...
async function main() {
    ...
    const MerkleInstance = await MerkleFactory.deploy(AlvareAddress, settings.merkleroot, networks[chainId].slth, networks[chainId].sama, samafactor);
    ...
    if (chainId == 56 || chainId == 97) {
      ...
      await run("verify:verify", {
        ...
        constructorArguments: [
            AlvareAddress, settings.merkleroot, networks[chainId].slth, networks[chainId].sama, samafactor
        ]
      })
    }
  }
